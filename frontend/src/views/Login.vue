...
function isUnauthorizedError(error: unknown) {
  ...
  return err?.response?.status === 401 ||
    ['认证令牌无效', '登录已过期', '未授权'].some(text => message.includes(text))
}

function promptOAuthRelogin() {
  authStore.clearSession()
  mode.value = 'password'
  MessagePlugin.warning('登录已过期，请重新登录后继续授权')
}
  ...
  try {
    ...
    if (authStore.isAuthenticated) {
      try {
        ...
        if (isUnauthorizedError(e)) promptOAuthRelogin()
        else MessagePlugin.error(getErrorMessage(e, '会话校验失败，请重新登录'))
      }
    }
...
async function handlePasswordLogin() {
  ...
  try {
    ...
    await afterLoginSuccess(res.user.role)
  } catch (e: unknown) { MessagePlugin.error(getErrorMessage(e, '登录失败')) }
...
async function handleRegister() {
  ...
  try {
    ...
    authStore.applySession(res)
    await afterLoginSuccess(res.user.role)
  } catch (e: unknown) { MessagePlugin.error(getErrorMessage(e, '注册失败')) }
  finally { submitting.value = false }
}
...
async function handleAuthorize(decision: 'approve' | 'deny') {
  ...
  try {
    ...
    MessagePlugin.error(getErrorMessage(e, '操作失败'))
  }
  finally { submitting.value = false }
}
...
async function afterLoginSuccess(role: string) {
  if (hasOAuthContext.value) {
    if (!oauthCtx.value) await loadOAuthContext()
    if (oauthCtx.value) mode.value = 'authorize'
    return
  }
  await router.push(role === 'admin' ? '/user/overview' : '/user/account')
}
